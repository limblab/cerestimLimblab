...
beginningOfGroup\n\tendOfGroup\n\tautoStimulus\n\tplay\n";
    ...
    if (!strcmp(cs_cmd, "connect")) {
		if(nrhs==1){
			...
			numEl=nRow*nCol;
			if numEl!=3{
				mexPrintf("optional inputs to connect MUST be a 3 element array, containing [pid, timeout, vid]")}
			if(nRow>nCol){
				myParams.pid=inVector[1];
				myParams.timeout=inVector[2];
				myParams.vid=inVector[3];
			}else {
				myParams.pid=inVector[0][1];
				myParams.timeout=inVector[0][2];
				myParams.vid=inVector[0][3];
			}
		}
    ...
    if (!strcmp(cs_cmd, "stim_max")) {
        ...
        retval=myStim.stimulusMaxValues(BMaximumValues * output, UINT8 rw, BOCVolt voltage, UINT8 amplitude, UINT16 width, 
		...
		*/	
		int retval = myStim.stimulusMaxValues(&maxValues, 1, BOCVOLT9_5, 215, 65535, 65535, 65535, 0xFFFFFFFF);
        if (retval != BSUCCESS) {
